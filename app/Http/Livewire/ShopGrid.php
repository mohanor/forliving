...
class ShopGrid extends Component
{
    ...
    {
        $title = "products.title_" . config('app.locale') . " as title";
        $this->products = DB::table('products')
                ->join('images', 'products.id', 'images.product_id')
                ->join('product_categories', 'products.id', 'product_categories.product_id')
                ->join('categories', 'product_categories.category_id', 'categories.id')
                ->where('categories.name_' . config('app.locale'), "=", $category)
    ...
    public function search()
    {
            ...
            if ($category != 'all')
            {
                $this->products = DB::table('products')
                                ...
                                ->join('categories', 'product_categories.category_id', 'categories.id')
                                ->where('categories.name_' . config('app.locale'), "=", $category)
                                ->where('products.title_' . config('app.locale'), 'like', '%' . $searchWord . '%')
                                ->where('images.type', "=", "primary")
                                ->select('products.id as product',$title, 'products.price', 'images.src')
                                ->get();
            }
    ...
    public function getSearchByPriceUp($price)
    {
        ...
        if(Session::get('category') != "all")
        {
            $this->products = DB::table('products')
                ...
                ->join('product_categories', 'products.id', 'product_categories.product_id')
                ->join('categories', 'product_categories.category_id', 'categories.id')
                ->where('categories.name_' . config('app.locale'), "=", Session::get('category'))
                ->where('images.type', "=", "primary")
                ->where('price', '>', $price)
                ->select('products.id as product', $title, 'products.price', 'images.src')
                ->get();
    ...
    public function getSearchByPriceBetween($min, $max)
    {
        ...
        if(Session::get('category') != "all")
        {
            $this->products = DB::table('products')
                ...
                ->join('product_categories', 'products.id', 'product_categories.product_id')
                ->join('categories', 'product_categories.category_id', 'categories.id')
                ->where('categories.name_' . config('app.locale') . ' as name', "=", Session::get('category'))
                ->where('images.type', "=", "primary")
    ...
    public function render()
    {
        ...
        $this->categories = DB::table('categories')
            ->select('name_' . config('app.locale') . ' as name')
            ->get();

        $this->nbr_products = $this->products->count();
        
        $cart = Cart::content();
        return view('livewire.shop-grid', compact('cart'));
    }
